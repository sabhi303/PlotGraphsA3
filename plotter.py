...
def plot_CDF(data, title, plt):
    ...
    plt.plot(bin_edges[1:], cdf, linestyle='-', linewidth=2, marker='o', markersize=3, color='b', label='CDF')
...
def plot_box(data, title, plt):
    plt.subplot(1, 2, 2)
    plt.boxplot(data, patch_artist=True, notch=True, vert=False, meanline=True, showmeans=True, showcaps=True, showbox=True, showfliers=True)
